...
bifactor_model <- "
# general factor
g =~ E1 + E6 + E7 + D1 + D6 + D7 + R1 + R3 + R7 + M1 + M3 + M9 + F3 + F6 + F10 + G5 + G2 + G9 + L5 + L6 + L7 + C2 + C3 + C6 + X4 + X8 + X9 + S3 + S4 + S5 + I1 + I2 + B1 + B3 + B5
...
### Redo with Confidence Intervals using bootstrapping

```{r}
#| echo: true
#| output: asis
#| id: exp2-reliabilty-bootstrapped
...
omega_fun <- function(fit) {
  out <- semTools::compRelSEM(
    fit,
    tau.eq = FALSE
  )
...
boot_omega <- lavaan::bootstrapLavaan(
  fit1,
  R = 2000,
  parallel = "snow",
  verbose = TRUE,
...
print(knitr::kable(results,digits = 3,caption="Composite Reliability (Omega) with 95% Confidence Intervals."))
